...
def _btn(text: str, callback: str | None = None, url: str | None = None, emoji: str | None = None) -> InlineKeyboardButton:
    return InlineKeyboardButton(text=text, callback_data=callback, url=url, icon_custom_emoji_id=icon(emoji) if emoji else None)


def _reply_btn(text: str, emoji: str | None = None) -> KeyboardButton:
    return KeyboardButton(text=text, icon_custom_emoji_id=icon(emoji) if emoji else None)


def main_menu_reply(is_admin: bool = False) -> ReplyKeyboardMarkup:
    rows = [
        [_reply_btn(MENU_SEARCH, "eye")],
        [_reply_btn(MENU_TASKS, "bell"), _reply_btn(MENU_PROFILE, "profile")],
        [_reply_btn(MENU_SUBSCRIPTION, "star"), _reply_btn(MENU_HELP, "info")],
    ]
    if is_admin:
        rows.append([_reply_btn(MENU_ADMIN, "settings")])
    return ReplyKeyboardMarkup(keyboard=rows, resize_keyboard=True, input_field_placeholder="Выберите раздел")


def main_menu_inline() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [_btn("Найти выгодное", "menu:search", emoji="eye")],
        [_btn("Мои мониторинги", "menu:tasks", emoji="bell"), _btn("Профиль", "menu:profile", emoji="profile")],
        [_btn("Подписка", "menu:subscription", emoji="star"), _btn("Помощь", "menu:help", emoji="info")],
    ])
...
def cancel_kb(callback: str = "search:cancel") -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[_btn("Отмена", callback, emoji="cross")]])
...
def locations_kb(locations: list[Location]) -> InlineKeyboardMarkup:
    ...
    builder.row(_btn("Отмена", "search:cancel", emoji="cross"))
...
def found_locations_kb(locations: list[Location]) -> InlineKeyboardMarkup:
    ...
    builder.adjust(1)
    builder.row(_btn("Ввести заново", "search:loc_manual", emoji="write"), _btn("Отмена", "search:cancel", emoji="cross"))
    return builder.as_markup()
...
def price_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [_btn("Отмена", "search:cancel", emoji="cross")],
    ])
...
def confirm_search_kb(can_monitor: bool) -> InlineKeyboardMarkup:
    rows = [[_btn("Найти сейчас", "search:run", emoji="eye")]]
    if can_monitor:
        rows.append([_btn("Найти и включить мониторинг", "search:run_monitor", emoji="bell")])
    else:
        rows.append([_btn("Мониторинг (нужна подписка)", "search:need_sub", emoji="lock")])
    rows.append([_btn("Изменить запрос", "search:restart", emoji="edit"), _btn("Отмена", "search:cancel", emoji="cross")])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def listing_kb(listing: Listing, request_hash: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [_btn("Открыть на Авито", url=listing.url, emoji="link")],
...
def results_footer_kb(can_monitor: bool) -> InlineKeyboardMarkup:
    rows = []
    if can_monitor:
        rows.append([_btn("Включить мониторинг по этому запросу", "search:monitor_last", emoji="bell")])
    else:
        rows.append([_btn("Мониторинг — оформить подписку", "menu:subscription", emoji="star")])
...
def tasks_list_kb(tasks: list[SearchTask], can_add: bool) -> InlineKeyboardMarkup:
    ...
    builder.adjust(1)
    if can_add:
        builder.row(_btn("Добавить мониторинг", "menu:search", emoji="bell"))
    builder.row(_btn(BACK_TEXT, "menu:main"))
    return builder.as_markup()


def task_kb(task: SearchTask) -> InlineKeyboardMarkup:
    toggle = _btn("Поставить на паузу", f"task_toggle:{task.id}", emoji="lock") if task.is_active else _btn("Возобновить", f"task_toggle:{task.id}", emoji="unlock")
    return InlineKeyboardMarkup(inline_keyboard=[
        [toggle],
        [_btn("Мин. оценка", f"task_rating:{task.id}", emoji="stats"), _btn("Удалить", f"task_del:{task.id}", emoji="trash")],
...
def task_delete_confirm_kb(task_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [_btn("Да, удалить", f"task_del_yes:{task_id}", emoji="trash"), _btn("Нет", f"task:{task_id}", emoji="cross")],
    ])
...
def profile_kb(has_subscription: bool) -> InlineKeyboardMarkup:
    rows = [
        [_btn("Продлить подписку" if has_subscription else "Оформить подписку", "menu:subscription", emoji="star")],
        [_btn("Пополнить баланс", "topup:start", emoji="wallet"), _btn("Мои мониторинги", "menu:tasks", emoji="bell")],
...
def pay_methods_kb(tariff: Tariff, settings: RuntimeSettings, balance: float) -> InlineKeyboardMarkup:
    ...
    if balance >= tariff.price_rub:
        rows.append([_btn(f"Оплатить с баланса ({tariff.price_rub} ₽)", f"pay:{tariff.id}:balance", emoji="wallet")])
    if settings.stars_enabled:
        rows.append([_btn(f"Telegram Stars ({tariff.price_stars} ⭐)", f"pay:{tariff.id}:stars", emoji="star")])
    if settings.cryptobot_enabled and settings.cryptobot_token:
        rows.append([_btn("CryptoBot (крипта)", f"pay:{tariff.id}:cryptobot", emoji="cryptobot")])
    if not rows:
...
def topup_methods_kb(amount: int, settings: RuntimeSettings, stars_amount: int) -> InlineKeyboardMarkup:
    rows = []
    if settings.stars_enabled:
        rows.append([_btn(f"Telegram Stars ({stars_amount} ⭐)", f"topup:pay:{amount}:stars", emoji="star")])
    if settings.cryptobot_enabled and settings.cryptobot_token:
        rows.append([_btn("CryptoBot (крипта)", f"topup:pay:{amount}:cryptobot", emoji="cryptobot")])
    if not rows:
...
def crypto_invoice_kb(invoice_url: str, payment_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [_btn("Оплатить через CryptoBot", url=invoice_url, emoji="cryptobot")],
        [_btn("Проверить оплату", f"check_pay:{payment_id}", emoji="loading")],
        [_btn("Отменить", f"cancel_pay:{payment_id}", emoji="cross")],
    ])
...
def help_kb(support_username: str) -> InlineKeyboardMarkup:
    rows = []
    if support_username:
        rows.append([_btn("Написать в поддержку", url=f"https://t.me/{support_username}", emoji="write")])
