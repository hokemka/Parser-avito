...
def admin_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [_btn("Статистика", "admin:stats", emoji="stats"), _btn("Рассылка", "admin:broadcast", emoji="megaphone")],
...
def broadcast_buttons_kb(has_buttons: bool) -> InlineKeyboardMarkup:
    rows = [[_btn("Добавить кнопку", "bc:add_button", emoji="link")]]
    if has_buttons:
        rows.append([_btn("Удалить последнюю", "bc:remove_button", emoji="trash")])
    rows.append([_btn("Продолжить" if has_buttons else "Нет, продолжить", "bc:continue", emoji="check")])
    rows.append([_btn("Отмена", "bc:cancel", emoji="cross")])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def broadcast_confirm_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [_btn("Разослать", "bc:send", emoji="megaphone"), _btn("Нет", "bc:cancel", emoji="cross")],
    ])
...
def users_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [_btn("Поиск пользователя", "users:search", emoji="eye")],
...
def user_card_kb(user: User, has_subscription: bool) -> InlineKeyboardMarkup:
    ban = _btn("Разблокировать", f"users:unban:{user.id}", emoji="unlock") if user.is_banned else _btn("Забанить", f"users:ban:{user.id}", emoji="lock")
    rows = [
        [_btn("Изменить баланс", f"users:balance:{user.id}", emoji="wallet"), ban],
        [_btn("Выдать подписку", f"users:grant:{user.id}", emoji="gift")],
    ]
    if has_subscription:
        rows.append([_btn("Снять подписку", f"users:revoke:{user.id}", emoji="cross")])
...
def tariffs_admin_kb(tariffs: list[Tariff]) -> InlineKeyboardMarkup:
    ...
    builder.adjust(1)
    builder.row(_btn("Добавить тариф", "tariffs:add", emoji="gift"))
    builder.row(_btn(BACK_TEXT, "admin:menu"))
    return builder.as_markup()


def tariff_card_kb(tariff: Tariff) -> InlineKeyboardMarkup:
    toggle = _btn("Скрыть", f"tariffs:toggle:{tariff.id}", emoji="hidden") if tariff.is_active else _btn("Показать", f"tariffs:toggle:{tariff.id}", emoji="eye")
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [toggle, _btn("Удалить", f"tariffs:delete:{tariff.id}", emoji="trash")],
...
def tariff_delete_kb(tariff_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [_btn("Да, удалить", f"tariffs:delete_yes:{tariff_id}", emoji="trash"), _btn("Нет", f"tariffs:card:{tariff_id}", emoji="cross")],
    ])
...
def tariff_wizard_kb(skip_callback: str | None = None) -> InlineKeyboardMarkup:
    ...
    rows.append([_btn("Отмена", "admin:tariffs", emoji="cross")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def payment_settings_kb(settings: RuntimeSettings) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [_btn(f"Stars: {_flag(settings.stars_enabled)}", "settings:toggle:stars_enabled", emoji="green" if settings.stars_enabled else "red")],
        [_btn(f"CryptoBot: {_flag(settings.cryptobot_enabled)}", "settings:toggle:cryptobot_enabled", emoji="green" if settings.cryptobot_enabled else "red")],
        ...
        [_btn("Проверить CryptoBot", "settings:check_cryptobot", emoji="verify")],
        [_btn(BACK_TEXT, "admin:menu")],
    ])
...
def general_settings_kb(settings: RuntimeSettings) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [_btn(f"Премиум-эмодзи: {_flag(settings.premium_emoji)}", "settings:toggle:premium_emoji", emoji="brush")],
        [_btn(BACK_TEXT, "admin:menu")],
    ])
...
def parser_settings_kb(settings: RuntimeSettings) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [_btn("Модель ИИ", "settings:edit:ai_model", emoji="bot"), _btn(f"Фото в ИИ: {_flag(settings.ai_analyze_images)}", "settings:toggle:ai_analyze_images", emoji="photo")],
